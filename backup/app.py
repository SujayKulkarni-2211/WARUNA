...
# Function to create SQLite database and tables with dummy employee data
def create_tables():
    ...
    # Create employees table with dummy data
    ...
    c.execute("INSERT INTO employees VALUES ('manager1', 'password', 'Manager', 'manager1@example.com', '1234567890', 0, 0)")
    c.execute("INSERT INTO employees VALUES ('inspector1', 'password', 'Water Inspector', 'inspector1@example.com', '9876543210', 0, 0)")
...
def add_to_reported_problems(issue_type, issue_description, latitude, longitude):
    ...
    c.execute("INSERT INTO reported_problems (location, issue_description, lat, lng) VALUES (?, ?, ?, ?)",
              (issue_type, issue_description, latitude, longitude))
    conn.commit()
    conn.close()
...
if __name__ == "__main__":
    app.run(port=5001, debug=True)  # Change the port number to 5001
